...
class UIElementsTree:
    def __init__(self, application="System Events", process="System Preferences"):
        ...
        self.__apllescript_object_types = ["application processes", "application process", "UI element", "pop over", "window", "anchor", "scroll area", "tab group", "splitter group", "static text", "sheet", "table", "checkbox", "button", "text field", "group", "scroll bar", "toolbar", "image", "menu bar item", "menu bar", "menu item", "menu", "application", "list", "row", "column"]
    ...
    def getFinalElementObject(self, objStr=None, index=None):
        object_ = None

        if index == None:
            object_ = self.find(objStr)
        else:
            object_ = self.findAll(objStr, process=self.process)
            if object_ == {} or self.process not in object_:
                print('unknown...\n\n\n')
                object_ = None
            else:
                object_ = object_[self.process][index]

        return object_

    def generateEventScript(self, event='click', objStr=None, option=None, index=None, text='test', x=0, y=0):
        # object_ = self.find(objStr)
        object_ = self.getFinalElementObject(objStr, index)
    ...
    def sendEvent(self, event='click', objStr=None, option=None, index=None, text='test', x=0, y=0):
        res = False

        cmd = self.generateEventScript(event, objStr, option=option, index=index, text=text, x=x, y=y)
        if cmd == None:
            return False
...
# afindAllElement(process='System Preferences', element = 'button 2')
aclick(process='System Preferences', object='button Sign In', timeout=1, throw=False, index=1)